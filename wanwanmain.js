...
function AddExp( val )
{
	...
	while( gLv * ( gLv + 1 ) * 2 <= gEx ){				//	レベルアップ条件を満たしている場合
		gLv++;											//	レベルアップ
		gMHP += 4 + (Math.floor( Math.random() * 3 )) * 2;	//	最大HP上昇10～12
	}
}
...
function DrawStatus( g )
{
	g.font = FONT;									//	文字フォントを設定
	g.fillStyle = FONTSTYLE;						//	文字色
	g.fillText( "Lv", 14, 39 );	DrawTextR( g, gLv, 125, 39 );	//	Lv
	g.fillText( "HP", 14, 75 );	DrawTextR( g, gHP, 125, 75 );	//	HP
	g.fillText( "Ex", 14, 111 );	DrawTextR( g, gEx, 125, 111 );	//	Ex
}
